...
extract_manifest_version() {
  ...
  if [[ "${raw}" == 'version.workspace=true' || "${raw}" == 'version={workspace=true}' ]]; then
    # Resolve to the workspace version in the root Cargo.toml
    extract_workspace_version "${ROOT_DIR}/Cargo.toml"
  else
    # Strip version="..." wrapper
    echo "${raw}" | sed 's/^version="//;s/"$//'
  fi
}
...
main() {
  ...
  if [[ -z "${VERSION}" ]]; then
    VERSION="$(extract_workspace_version "${ROOT_DIR}/Cargo.toml")"
  fi
