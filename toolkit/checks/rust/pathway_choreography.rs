        ...
        for required in [
            ...
            if !contents.contains(required) {
                out.push(Violation::with_layer(
                    rel.clone(),
                    1,
                    format!("inline pathway choreography module must contain `{required}`"),
                    layer_for_rel_path(&rel),
                ));
            }
        }
...
fn forbidden_runtime_effect_calls(root: &Path) -> Result<Vec<Violation>> {
    let patterns = [
        (
            "self.transport.send_transport(",
            "pathway runtime must send transport payloads through choreography guest runtime",
        ),
        (
            "self.transport.drain_transport_ingress(",
            "pathway runtime must drain ingress through choreography guest runtime",
        ),
        (
            "self.retention.retain_payload(",
            "pathway runtime must retain payloads through choreography guest runtime",
        ),
        (
            "self.retention.take_retained_payload(",
            "pathway runtime must recover retained payloads through choreography guest runtime",
        ),
        (
            "self.effects.record_route_event(",
            "pathway runtime must record route events through choreography guest runtime",
        ),
    ];
    let mut out = Vec::new();
...
fn no_pathway_choreography_types_in_shared_crates(root: &Path) -> Result<Vec<Violation>> {
    let mut out = Vec::new();
    for dir in ["crates/core/src", "crates/traits/src"] {
        for path in rust_files(&root.join(dir))? {
            let rel = normalize_rel_path(root, &path);
...
fn router_does_not_depend_on_pathway_private_choreography(root: &Path) -> Result<Vec<Violation>> {
