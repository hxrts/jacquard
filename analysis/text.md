...
- `mercator` is carried as a standalone comparison peer through those same route-visible regimes: {mercator_connected_high_loss_route_presence} permille in `connected-high-loss`, {mercator_bridge_transition_route_presence} permille in `bridge-transition`, {mercator_corridor_uncertainty_route_presence} permille in `corridor-continuity-uncertainty`, and {mercator_concurrent_mixed_route_presence} permille in `concurrent-mixed`.
