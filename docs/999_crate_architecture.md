...
| `jacquard-field` | Field-private posterior state, mean-field compression, regime/posture control state, bounded runtime-round diagnostics, coded-diffusion research vocabulary and scaffolding, and any field-private choreography runtime used only for observational summary exchange. Legacy Telltale-backed frozen-snapshot route search and continuation scoring remain baseline-only compatibility surfaces. |
...
For first-party field, the proof and ownership boundary is even stricter. Field-private choreography may supply only observational evidence into the deterministic local controller. It must not publish canonical route truth or leak field-private session semantics into shared router surfaces. The coded-diffusion research reset uses Field as an initial workspace, but the research program is name-independent and must be separable from Jacquard and Field.
