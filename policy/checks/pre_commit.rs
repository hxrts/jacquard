...
fn ensure_no_gitignored_files(root: &Path, staged_files: &[String]) -> Result<()> {
    ...
    if !ignored.is_empty() {
        ...
        eprintln!("pre-commit: do not use --no-verify to override this guard.");
        eprintln!(
            "pre-commit: gitignored files are local-only scratch/output files and must not be added to source control."
        );
        eprintln!("pre-commit: remove them from the index instead:");
        eprintln!("  git reset HEAD <file>");
        eprintln!("  git rm --cached <file>    # if the file is already tracked");
...
fn run_optional_format_check(root: &Path, crates: &BTreeSet<String>) -> Result<()> {
    ...
    let mut command = Command::new(root.join("scripts/toolkit-shell.sh"));
    command.arg("toolkit-fmt");
    command.args(package_args(crates));
    command.args(["--", "--check"]);
    command.current_dir(root);
    let status = command.status().context("running workspace formatter")?;
    if !status.success() {
