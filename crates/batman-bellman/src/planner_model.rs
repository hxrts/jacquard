//! Engine-owned pure planner model surface for BATMAN Bellman.
...
use crate::{
    admit_route_from_snapshot, candidate_routes_from_snapshot, BatmanBellmanPlannerSnapshot,
    BestNextHop, DecayWindow,
};

#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub struct BatmanBellmanPlannerSeed {
    pub local_node_id: NodeId,
    pub selected_neighbor: NodeId,
}

pub struct BatmanBellmanPlannerModel;

impl RoutingEnginePlannerModel for BatmanBellmanPlannerModel {
    type PlannerSnapshot = BatmanBellmanPlannerSeed;
...
fn planner_snapshot(
    ...
    let jacquard_core::DestinationId::Node(destination) = objective.destination else {
        return BatmanBellmanPlannerSnapshot {
