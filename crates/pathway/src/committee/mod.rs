...
pub(crate) const PATHWAY_COMMITTEE_MEMBERSHIP_CAP: u8 = 3;
...
const PATHWAY_COMMITTEE_BEHAVIOR_RELIABILITY_FLOOR: u32 = 400;
const PATHWAY_COMMITTEE_BEHAVIOR_PENALTY_CEILING: u32 = 400;

#[derive(Clone, Debug, PartialEq, Eq)]
pub(crate) struct PathwayBehaviorHistory {
...
#[must_use]
pub fn pathway_admission_assumptions(
...
mod tests {
    ...
    #[test]
    fn pathway_health_score_is_clamped_to_health_score_max() {
        ...
        assert_eq!(
            pathway_health_score(&topology.value),
            HealthScore(HEALTH_SCORE_MAX)
        );
    }

    #[test]
    fn pathway_health_score_prefers_balanced_middle_regime_over_extremes() {
        ...
        let sparse_score = pathway_health_score(&sparse_calm.value);
        let dense_chaotic_score = pathway_health_score(&dense_chaotic.value);
        let balanced_score = pathway_health_score(&balanced.value);
