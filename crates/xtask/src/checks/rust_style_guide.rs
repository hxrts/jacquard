...
fn collect_violations() -> Result<Vec<Violation>> {
    ...
    for source in &parsed {
        violations.extend(scan_function_lengths(source));
...
fn scan_function_lengths(source: &ParsedSource) -> Vec<Violation> {
    ...
    for function in fn_collector.functions {
        let function_line_count =
            function.end_line.saturating_sub(function.start_line) + 1;
        if function_line_count <= FUNCTION_LINE_MAX {
            continue;
        }
        if has_nearby_exception_marker(
            &source_lines,
            function.start_line,
            function.end_line,
        ) || is_style_exempt(&function.symbol)
        {
            continue;
        }
...
fn has_nearby_exception_marker(
    source_lines: &[&str],
    start_line: usize,
    end_line: usize,
) -> bool {
...
fn is_style_exempt(symbol: &str) -> bool {
    STYLE_GUIDE_EXCEPTIONS
        .iter()
