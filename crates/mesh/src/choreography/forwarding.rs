...
pub(crate) const SOURCE_PATH: &str = "crates/mesh/src/choreography/forwarding.rs";
...
impl<E> effects::MeshRuntime for NextHopHost<'_, E>
where
    E: MeshProtocolRuntime,
{
    ...
    ) -> Result<effects::ForwardReceipt, effects::MeshProtocolError> {
    ...
    fn record(&mut self, input: effects::AuditEvent) {
        ...
        shared
            ...
            .emit_protocol_observation(MeshProtocolObservation {
                protocol: MeshProtocolKind::ForwardingHop,
                protocol_name: spec.protocol_name.clone(),
                role_names: spec.role_names.clone(),
                session: route_session(MeshProtocolKind::ForwardingHop, &route_id),
                detail,
            });
    }
}
    ...
    E: MeshProtocolRuntime,
{
    try_session(role, |s: NextHopSession<'_, _>| async {
        ...
        let outcome = effects::MeshRuntime::forward_frame(
            host,
            effects::HopFrame {
...
mod tests {
    ...
    impl MeshProtocolRuntime for FakeEffects {
        ...
        fn store_protocol_checkpoint(
            &mut self,
            _checkpoint: &MeshCheckpointEnvelope,
