...
//!
//! Identical in structure to the enhanced batman planner. Candidate
//! enumeration reads the best next-hop table; admission validates that the
//! candidate's `BackendRouteRef` still matches the current best next-hop.
...
use crate::{BatmanClassicEngine, BATMAN_CLASSIC_CAPABILITIES, BATMAN_CLASSIC_ENGINE_ID};
