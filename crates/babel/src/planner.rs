//! `RoutingEnginePlanner` impl for `BabelEngine`.
//!
//! Identical in structure to the batman-classic planner. Candidate enumeration
//! reads the best next-hop table; admission validates that the candidate's
//! `BackendRouteRef` still matches the current best next-hop. Destination
//! service support is verified against `BABEL_ENGINE_ID` in the node profile
...
use crate::{
    ...
    BabelEngine, BabelPlannerSnapshot, BABEL_CAPABILITIES, BABEL_ENGINE_ID,
};
...
impl<Transport, Effects> RoutingEnginePlannerModel for BabelEngine<Transport, Effects> {
    type PlannerSnapshot = BabelPlannerSnapshot;
        ...
        .map(|node| {
            node.profile.services.iter().any(|service| {
                service.service_kind == service_kind
                    && service.routing_engines.contains(&BABEL_ENGINE_ID)
            })
        })
        .unwrap_or(false)
}
...
mod tests {
    ...
    #[test]
    fn candidate_routes_require_babel_engine_id_in_destination_service() {
        ...
        // Inject an update for node(2) so the engine would otherwise produce a candidate.
        let update = crate::gossip::BabelUpdate {
            destination: node(2),
            router_id: node(2),
            seqno: 1,
            metric: 0,
