...
`Protocol/Bridge.lean` connects the reduced protocol object to a Telltale-shaped machine fragment, defining the snapshot-to-fragment relation, fragment semantic-object traces, and a simulation-style bridge from reduced machine steps to reduced fragment steps. `Protocol/Conservation.lean` proves field-side evidence conservation over controller-visible traces and snapshot and fragment authority conservation wrappers. `Protocol/Coherence.lean` proves the reduced updated-edge, incident-edge, and unrelated-edge style coherence cases. `Protocol/ReceiveRefinement.lean` introduces the smallest typed receive-refinement hook over summary and ack receives. `Protocol/Reconfiguration.lean` audits the fixed-participant property and explicitly rules out reconfiguration from the current protocol semantics.
